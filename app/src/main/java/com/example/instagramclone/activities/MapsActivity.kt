...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listStory.forEach { item ->
            if (item.lat > 180 || item.lon > 180 || item.lat < -180 || item.lon < -180) {
                Log.d(TAG, "LAT ABOVE: ${item.lat}")
                Log.d(TAG, "LON ABOVE: ${item.lon}")
            } else {
                Log.d(TAG, "LAT: ${item.lat}")
                Log.d(TAG, "LON: ${item.lon}")
            }

//            val latLng = LatLng(item.lat, item.lon)
//            Log.d("TEST",latLng.toString())
//            val addressName = getAddressName(item.lat, item.lon)
//            Log.d(TAG, "onCreate: $addressName")
...
////            boundsBuilder.include(latLng)
    ...
    private fun addManyMarker() {
        ...
        listStory.forEach { item ->
            if (item.lat > 180 || item.lon > 180 || item.lat < -180 || item.lon < -180) {
                Log.d(TAG, "addManyMarker: Out of bound")
            } else {
                val latLng = LatLng(item.lat, item.lon)
                Log.d("TEST",latLng.toString())
                val addressName = getAddressName(item.lat, item.lon)
